    ...
    internal static class ReflectionExtensions
    {
        ...
        public static Dictionary<string, PropertyData> GetProperties(this Type type)
        {
            ...
            if (!typeProperties.TryGetValue(type, out Dictionary<string, PropertyData> properties))
            {
                ...
                {
                    properties.Add(propertyInfo.Name, new PropertyData(propertyInfo, propertyInfo.GetCustomAttributes<TransformationAttribute>(true).Cast<TransformationAttribute>().OrderBy(x => x.TransformationPriority).ToList()));
                }
